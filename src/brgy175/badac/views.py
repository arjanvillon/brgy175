...
class BADACCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not query:
            case_no_badac = "K-2019-0000"
        else:
            pk = query.pk + 1
            year = datetime.datetime.now().year

            if pk < 10:
                case_no_badac = "K-" + str(year) + "-000" + str(pk)
            elif pk >= 10 and pk < 100:
                case_no_badac = "K-" + str(year) + "-00" + str(pk)
            elif pk >= 100 and pk < 1000:
                case_no_badac = "K-" + str(year) + "-0" + str(pk)
            else:
                case_no_badac = "K-" + str(year) + "-" + str(pk)
